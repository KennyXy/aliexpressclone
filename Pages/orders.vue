<template>
  <MainLayout class="max-h-screen overflow-x-scroll">
    <div id="OrdersPage" class="-mt-18 max-w-[1200px] mx-auto px-2 min-h-[50vh]">
        ...
        >
          <div class="border-b py-1">
            ...
            <div v-for="item in order.orderItem">
              <NuxtLink
                class="flex items-center gap-3 p-1 hover:uderline hover:text-blue-500"
                :to="`/item/${item.productId}`"
              >
                <img width="40" :src="item.product.image" alt="" />
            ...
            <div class="pt-2 pb-5">
              ...
              {{ order.zipcode }} , {{ order.country }} , {{ order.city }}
            </div>
          </div>
        </div>
